...
class IvyResolveTest(JvmToolTaskTestBase):
  ...
  def test_resolve_conflicted(self):
    ...
    #This test previously passed because the losing dep itself was
    #treated as added even though its deps would not have been added.

    #And in the non-isolated context, the SUT works because something must
    #depend on the selected version, even if we ourselves do not.

    #It's only in the isolated context that shit gets weird.

    #So in order to fake the isolated context, it would be nice
    #if we could take the ivyinfo and add some deps.
